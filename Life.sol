...
contract Life is ERC20, Ownable, Pausable {
    ...
    uint256 public burningRate = 1000000000000000;
    ...
    function mintInvestorsRewards(uint256 _amount, uint256 tokenId) external {
        require(
            _amount <= _investorsNft.checkClaimableStakingRewards(tokenId),
            "You can't claim that amount of rewards"
        );
        _mint(_investorsNft.ownerOfTokenId(tokenId), _amount);
    }
    ...
    function claimStakingRewards(uint256 tokenId) public whenNotPaused {
        ...
        // If the answer on tha above questions are true,
        ...
        uint256 amountToBurn = claimableStakingRewards(tokenId) * burningRate;
    ...
    modifier onlyInvestor(uint256 tokenId) {
        require(
            _investorsNft.ownerOf(tokenId) != msg.sender,
            "Only for NFTIs owner"
        );
        _;
    }
}
